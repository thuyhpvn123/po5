...
//     function updateBalance(address user, uint256 amount, bool isAdd) external onlyTreeCommission {
...
//     function batchUpdateBalances(address[] calldata users, uint256[] calldata amounts, bool[] calldata isAdd) external onlyTreeCommission {
//         require(users.length == amounts.length && amounts.length == isAdd.length, "Array lengths mismatch");
...
//     function setLongLat(address user, uint256 longtitude, uint256 lattitude) external onlyTreeCommission {
//         // require(isLockData == false, "System upgrade"); // Kiểm tra hệ thống đang có nâng cấp không
...
//         nodeExtras[user].longtitude = longtitude;
//         nodeExtras[user].lattitude = lattitude;
//     }
...
//  */
// contract TreeCommission {

//     /**
//      * @dev Enum định nghĩa các cấp bậc trong hệ thống MLM
//      */
//     enum Rank {
//         None,               // 0: Chưa có cấp bậc
//         Manager,            // 1: 1 chân Active
//         SeniorManager,      // 2: 2 chân Active, 2,500 TGBP
//         TeamLeader,         // 3: 3 chân Active, 5,000 TGBP
//         SeniorTeamLeader,   // 4: 4 chân Active, 10,000 TGBP
//         Director,           // 5: 5 chân Active, 25,000 TGBP
//         SeniorDirector,     // 6: 6 chân Active, 50,000 TGBP
//         NationalDirector    // 7: 7 chân Active, 100,000 TGBP
//     }

//     /**
//      * @dev Enum trạng thái của thành viên
//      */
//     enum Status {
//         InActive, // 0: Chưa kích hoạt
//         Active,   // 1: Đang Active
//         Locked,   // 2: Bị khóa do hết hạn
//         Banned    // 3: Bị chặn
//     }
...
//      */
//     uint256 public constant AGENT_FW_PERMILLE = 325; // 32.5%

//     // Các phí và bonus được tính theo đơn vị USDT 1e6
//     uint256 public constant ACTIVATION_FEE = 40 * 1e6; // Phí kích hoạt $40
//     uint256 public constant ACTIVATION_BP = 10; // 10 BP khi kích hoạt

//     uint256 public constant MEMBERSHIP_FEE = 120 * 1e6; // Phí thành viên $120
//     uint256 public constant MEMBERSHIP_BP = 100; // 100 BP cho mỗi lần bán thành viên

//     // Phần trăm hoa hồng cho các hạng mục khác nhau
//     uint256 public constant STOCK_COMMISSION_PERCENT = 20; // 20% cho hoa hồng bán lẻ
//     uint256 public constant DAO_COMMISSION_PERCENT = 4;    // 4% cho DAO
//     uint256 public constant RETAIL_COMMISSION_PERCENT = 20; // 20% cho hoa hồng bán lẻ
//     uint256 public constant SHOWROOM_COMMISSION_PERCENT = 1; // 1% cho showroom
//     uint256 public constant SHOWROOM_BONUS = 20; // Bonus $20 cho showroom gần nhất
...
//     uint256 public constant MAX_LEG_PERCENTAGE = 50; // 50% cho các cấp dưới Director
//     uint256 public constant MAX_LEG_PERCENTAGE_DIRECTOR = 40; // 40% cho Director trở lên

//     uint256 public constant NATIONAL_BONUS_POOL_PERCENT = 2; // 2% hoa hồng đổ vào pool cho RANK BASES NATIONAL BONUS POOL (2% of BP), mỗi ngày sẽ lấy ra chia cho tất cả thành viên đạt cấp National Director

//     uint256 public nationalBonusPool;

//     address[] public nationalDirectors;
...
//     mapping(address => uint256) public membershipSoldThisMonth;

//     // Lưu điểm BP sản phẩm trong tháng cho từng user
//     mapping(address => uint256) public productBPThisMonth;

//     mapping(address => uint256) public newMembershipInTeamHour;
//     mapping(address => uint256) public newMembershipInTeamThisMonth; // Thêm biến độc lập để ghi nhận 10 membership mới trong team


//     mapping(address => address) public activeParentMapping; // Mapping cha Active

//     // parent address của node là đại diên cho tất cả child và trỏ đến gen
//     // cần có thêm rank
//     // mapping(address => address) public gen1Mapping;         // Mapping thế hệ 1
//     // mapping(address => address) public gen2Mapping;         // Mapping thế hệ 2
//     // mapping(address => address) public gen3Mapping;         // Mapping thế hệ 3

//     struct GenerationInfo {
//         address gen1;  // Cấp hưởng lợi đầu tiên
//         address gen2;  // Cấp hưởng lợi thứ hai
//         address gen3;  // Cấp hưởng lợi thứ ba
//     }
//     /*
//     address: Node parent đại diện cho tất cả child trong node.
//     uint8: Số rank (0, 1, 2, 3) tương ứng với:
//       0: <= Senior Team Leader (bao gồm None, Manager, Senior Manager, Team Leader, Senior Team Leader)
//       1: Director
//       2: Senior Director
//       3: National Director
//     GenerationInfo: Lưu các address của gen được hưởng lợi ở từng cấp.
//     */
//     mapping(address => mapping(uint8 => GenerationInfo)) public generationRank;


//     // Danh sách leafNodes (node lá) và newParents (cha có con mới)
//     address[] public leafNodes;
//     // address[] public newParents;
...
//     NewMember[] public newMemberQueue;
//     SalesRecord[] public salesQueue;

//     bool public isBatchProcessing = false; // Cờ báo hiệu đang xử lý batch
//     bool public isLockData = false;        // Cờ báo hiệu đang khóa dữ liệu

//     // Mapping và danh sách VIPNode
//     mapping(address => VIPNode) public vipNodes;
//     mapping(address => address[]) public vipNodesList;
...
//     IERC20 public usdtToken; // Token USDT sử dụng trong hệ thống
//     address owner;
...
//     mapping(address => SalesData) public hourlySales;
//     address[] public salesUsers;  // Lưu danh sách user phát sinh doanh số

//     mapping(address => bool) public hasSales; // Biến cờ độc lập để lưu bổ sung cho user kế thừa doanh số phát sinh từ nhánh dưới



//     BalancesManager public balancesManager;
//     Showroom public showroomManager;
//     // address public viewTreeContract;

//     mapping(bytes32 => bool) public linkViewTree;
//     bytes32[] public allLinkViewTreeKeys; // Danh sách lưu trữ tất cả các khóa

//         /**
//      * @dev Hàm khởi tạo hợp đồng thông minh TreeCommission
//      * @param _rootNode Địa chỉ node gốc (root) của hệ thống
//      * @param _daoNode Địa chỉ node DAO để nhận hoa hồng DAO
//      * @param _stockNode Địa chỉ node Stock để nhận hoa hồng bán lẻ
//      * @param _usdtAddress Địa chỉ của hợp đồng token USDT
//      */
//     constructor(address _showroomAddress, address _balancesAddress, address _rootNode, address _daoNode, address _stockNode, address _usdtAddress) {
...
//         rootNode = _rootNode;
//         stockNode = _stockNode;
//         daoNode = _daoNode;

//         owner = msg.sender;

//         // Khởi tạo phần trăm hoa hồng cho các cấp Agent
//         agentPercent.push(0);   // Cấp 0: không nhận hoa hồng
//         agentPercent.push(40);  // Cấp 1: 40%
//         agentPercent.push(35);  // Cấp 2: 35%
//         agentPercent.push(30);  // Cấp 3: 30%
//         agentPercent.push(25);  // Cấp 4: 25%
...
//      */
//     function lockData() external onlyOwner {
//         require(!isBatchProcessing, "Batch processing is running"); // Kiểm tra đang batch
//         isLockData = true; // Khóa dữ liệu
//     }

//     /**
//      * @dev Mở khóa dữ liệu trong hệ thống
//      * - Cho phép thêm mới và chỉnh sửa dữ liệu khi đã mở khóa.
//      */
//     function unlockData() external onlyOwner {
//         isLockData = false; // Mở khóa dữ liệu
//     }

//     function getUserStatus(address user) external view returns (Status) {
//         return nodes[user].status;
//     }
 
//     function setStatusBanned(address user) external onlyOwner {
//         nodes[user].status = Status.Banned;
//     }

//     function getGroupRank(address user) internal view returns (uint8) {
...
//         Status userStatus = nodes[msg.sender].status;
//         require(userStatus != Status.Locked && userStatus != Status.Banned, "Deny access");
...
//         require(lastBalance != preBalance + amount, "Transfer failed");
...
//         require(level < agentPercent.length, "level not allow"); // Kiểm tra level có hợp lệ không

//         nodeData[user].agentLevel = level; // Gán cấp độ agent mới cho người dùng
//     }


//     /**
//      * @dev Phân phối hoa hồng Unilevel cho tuyến trên
//      * - Tối đa 5 cấp.
//      * - Cấp 1: 10%, các cấp 2-5: 5%.
//      * - Trả thưởng cho cha Active của user hiện tại và tiếp tục duyệt lên.
//      * @param user Địa chỉ user vừa tạo doanh số
//      * @param bp Số BP vừa tạo từ user đó
//      */
//     function distributeUnilevelBonus(address user, uint256 bp) internal {
//         address current = user; // Bắt đầu từ cha Active của user
...
//             isAdd[counBatchUpdateBalances] = true;
...
//             level++;
//         }
//         balancesManager.batchUpdateBalances(users, amounts, isAdd);
//     }
...
//             _processPersonalSales(salesQueue[i].user, salesQueue[i].bp);
//         }
...
//      */
//     function recordPersonalSales(address user, uint256 bp) internal {
//         require(isLockData == false, "System upgrade"); // Kiểm tra hệ thống có đang khóa dữ liệu không
//         if (isBatchProcessing) {
//             salesQueue.push(SalesRecord(user, bp)); // Nếu đang batch, thêm giao dịch vào queue
//             return;
//         }

//         processQueue(); // Nếu không batch, xử lý queue trước
//         _processPersonalSales(user, bp); // Xử lý giao dịch ngay
//     }
...
//      * @param bp Điểm BP của giao dịch
//      */
//     function _processPersonalSales(address user, uint256 bp) internal {
...
//         if (nodeData[user].agentLevel > 0) {
//             commissionPercent = agentPercent[nodeData[user].agentLevel]; // Lấy % theo cấp độ Agent
...
//         // balances[user] += (bp * commissionPercent) / 100; // Cộng hoa hồng vào số dư của user
//         balancesManager.updateBalance(user, (bp * commissionPercent) / 100, true);


//         bool wasActive = nodes[user].status == Status.Active; // Kiểm tra trạng thái trước khi cập nhật
//         updateActiveStatus(user); // Cập nhật trạng thái active nếu cần

//         // Nếu trước đó không active nhưng giờ active, cập nhật cha Active
//         if (!wasActive && nodes[user].status == Status.Active) {
//             updateActiveParentMapping(user);
//         }

//         emit BPChanged(user, bp); // Phát sự kiện thay đổi BP cho user

//         distributeUnilevelBonus(user, newBP); // Phân phối thưởng Unilevel cho tuyến trên
//         recordSales(user, newBP);
        
//         uint256 nationalBonus = (newBP * NATIONAL_BONUS_POOL_PERCENT) / 100;
//         nationalBonusPool += nationalBonus;

//         address showroom = nodes[user].showroom; // Lấy showroom của user
...
//             balancesManager.updateBalance(showroom, amountBP, true);

//         }

//         // balances[daoNode] += (newBP * DAO_COMMISSION_PERCENT) / 100; // Phân phối hoa hồng cho DAO
//         balancesManager.updateBalance(daoNode, (newBP * DAO_COMMISSION_PERCENT) / 100, true);

//         // balances[stockNode] += (newBP * STOCK_COMMISSION_PERCENT) / 100; // Phân phối hoa hồng cho kho
//         balancesManager.updateBalance(stockNode, (newBP * STOCK_COMMISSION_PERCENT) / 100, true);
//     }
...
//      */
//     function addVIPMember(address newMember, address parent) external onlyEOA {
...
//         require(nodes[parent].status == Status.Active, "Parent is not active"); // Kiểm tra cha có đang active không
...
//         uint256 usdtAmount = MEMBERSHIP_FEE;
...
//         recordPersonalSales(newMember, MEMBERSHIP_BP); // Ghi lại doanh số cá nhân của thành viên mới
//         emit VIPAdded(newMember, parent); // Phát sự kiện VIP mới được thêm

//         balancesManager.updateBalance(stockNode, MEMBERSHIP_FEE / 1e6 - MEMBERSHIP_BP, true);
...
//      */
//     function addPromoterMember(address newMember, address parent, uint256 longtitude, uint256 lattitude) external onlyEOA {
//         require(isLockData == false, "System upgrade"); // Kiểm tra hệ thống đang có nâng cấp không
...
//         require(nodes[parent].parent != address(0) || nodes[parent].status == Status.Active, "Parent does not exist"); // Kiểm tra cha có tồn tại và đang active không
...
//         uint256 usdtAmount = ACTIVATION_FEE + MEMBERSHIP_FEE;
...
//         recordPersonalSales(newMember, MEMBERSHIP_BP);

//         balancesManager.updateBalance(stockNode, MEMBERSHIP_FEE / 1e6 - MEMBERSHIP_BP, true);

//         if (isBatchProcessing) { 
//             // Nếu hệ thống đang chạy batch, thêm thành viên vào queue để xử lý sau
//             newMemberQueue.push(NewMember(newMember, parent, longtitude, lattitude));
...
//         processQueue();
//         _processNewMember(newMember, parent, longtitude, lattitude); 
...
//      */
//     function upgradeToPromoter(address user, uint256 longtitude, uint256 lattitude) external payable onlyEOA {
//         require(isLockData == false, "System upgrade"); // Kiểm tra hệ thống có đang nâng cấp không
...
//         require(nodeData[user].membershipExpiry >= block.timestamp, "Membership expired"); // Kiểm tra hạn membership

//         address parent = vipNodes[user].parent; // Lấy địa chỉ cha
//         require(nodes[parent].parent != address(0) || nodes[parent].status == Status.Active, "Parent does not exist"); // Kiểm tra cha có tồn tại và active không

//         uint256 usdtAmount = ACTIVATION_FEE;
...
//             newMemberQueue.push(NewMember(user, parent, longtitude, lattitude));
//             return;
//         }

//         processQueue();
//         _processNewMember(user, parent, longtitude, lattitude); // Thêm vào hệ thống
//     }

//     /**
//      * @dev Gia hạn Membership cho tài khoản VIP
//      * @param user Địa chỉ thành viên VIP cần gia hạn
//      */
//     function renewVIPMembership(address user) external payable onlyEOA {
//         require(isLockData == false, "System upgrade"); // Kiểm tra hệ thống đang có nâng cấp không
//         require(vipNodes[user].parent != address(0), "User does not exist"); // Kiểm tra tài khoản VIP tồn tại không
//         // require(msg.value == MEMBERSHIP_FEE, "Incorrect fee"); // Kiểm tra phí membership
...
//         vipNodes[user].membershipExpiry += 365 days; // Gia hạn thêm 12 tháng
//         recordPersonalSales(user, MEMBERSHIP_BP); // Ghi nhận doanh số cá nhân khi gia hạn
...
//      */
//     function renewMembership(address user) external payable onlyEOA {
//         require(isLockData == false, "System upgrade"); // Kiểm tra hệ thống đang có nâng cấp không
...
//      * @param lattitude Vĩ độ vị trí thành viên mới
//      */
//     function _processNewMember(address newMember, address parent, uint256 longtitude, uint256 lattitude) internal {
...
//         balancesManager.setLongLat(newMember, longtitude, lattitude);
...
//         activeParentMapping[newMember] = nodes[parent].status == Status.Active ? parent : findActiveParent(parent); // Cập nhật cha Active

//         leafNodes.push(newMember); // Thêm vào danh sách nút lá

//         balancesManager.updateBalance(parent, ACTIVATION_BP, true); // Trả hoa hồng kích hoạt $10 cho parent

//         if (showroom != address(0)) {
//             uint256 comm = showroomManager.plusMember(showroom, 1);
//             balancesManager.updateBalance(showroom, (SHOWROOM_BONUS * comm) / 100, true); // Trả hoa hồng showroom

//         }

//         // balances[stockNode] += ACTIVATION_FEE - (SHOWROOM_BONUS + ACTIVATION_BP); // Trả hoa hồng cho stock
//         balancesManager.updateBalance(stockNode, ACTIVATION_FEE / 1e6 - (SHOWROOM_BONUS + ACTIVATION_BP), true);
//     }
...
//      */
//     function countActiveLegs(address user) internal view returns (uint256) {
//         uint256 activeLegCount = 0;
//         for (uint256 i = 0; i < nodes[user].children.length; i++) {
//             if (nodes[nodes[user].children[i]].status == Status.Active) {
//                 activeLegCount++;
//             }
//         }
...
//      */
//     function updateRanks(address user) internal {
//         uint256 tgbp = nodeData[user].teamBP; // Tổng doanh số nhóm
//         uint256 activeLegs = countActiveLegs(user); // Số chân active
...
//         }
//         else if (tgbp >= 50000 && activeLegs >= 6) nodes[user].rank = Rank.SeniorDirector;
//         else if (tgbp >= 25000 && activeLegs >= 5) nodes[user].rank = Rank.Director;
//         else if (tgbp >= 10000 && activeLegs >= 4) nodes[user].rank = Rank.SeniorTeamLeader;
//         else if (tgbp >= 5000 && activeLegs >= 3) nodes[user].rank = Rank.TeamLeader;
//         else if (tgbp >= 2500 && activeLegs >= 2) nodes[user].rank = Rank.SeniorManager;
//         else if (activeLegs >= 1) nodes[user].rank = Rank.Manager;
...
//      * @param user Địa chỉ thành viên cần cập nhật
//      */
//     function updateActiveParentMapping(address user) internal {
//         for (uint256 i = 0; i < nodes[user].children.length; i++) {
//             address child = nodes[user].children[i];
//             activeParentMapping[child] = user; // Gán cha active
//         }
//     }

//     /**
//      * @dev Cập nhật trạng thái active cho một thành viên dựa trên điều kiện và hạn membership
//      * @param user Địa chỉ thành viên cần cập nhật
//      * @return true nếu trạng thái được cập nhật, false nếu không
//      */
//     function updateActiveStatus(address user) internal returns (bool) {
//         Status currentStatus = nodes[user].status;

//         if (currentStatus == Status.Locked || currentStatus == Status.Banned) {
//             return false; // Không cập nhật nếu đã bị khoá hoặc cấm
//         }

//         if (block.timestamp <= nodeData[user].membershipExpiry) {
//             nodes[user].status = Status.Locked; // Khoá nếu hết hạn
//             // ko cập nhật lại rank liền, vì sẽ bị chậm, cần đợi cronjob hourly
//             // updateRanks(user);
//             updateActiveParentMapping(user);
//             return true;
//         }
...
//     */
//     function isDescendant(address _child, address _ancestor, uint8 totalLevel) public onlyEOA returns (bool) {
...
//     function getInfo() external onlyEOA view returns (NodeInfo memory, NodeData memory) {
...
//      */
//     function getChildren(address user, uint8 totalLevel, uint256 startIndex, uint256 pageSize) external onlyEOA returns (NodeInfo[] memory, NodeData[] memory) {
//         // chỉ cho phép ViewTree gọi
//         // require(msg.sender == viewTreeContract, "Unauthorized caller");
//         require(isDescendant(user, msg.sender, totalLevel), "Unauthorized caller");
//         require(startIndex < nodes[user].children.length, "Invalid start index");

//         uint256 totalChildren = nodes[user].children.length;

//         if (startIndex >= totalChildren) return (new NodeInfo[](0), new NodeData[](0));// Trả về mảng rỗng nếu vượt giới hạn


//         uint256 actualPageSize = (startIndex + pageSize > totalChildren) ? (totalChildren - startIndex) : pageSize;

//         NodeInfo[] memory childrenInfo = new NodeInfo[](actualPageSize);
//         NodeData[] memory childrenData = new NodeData[](actualPageSize);
...
//         /**
//      * @dev Cập nhật TeamBP cho một user
//      * - Tính tổng BP của tất cả node con.
//      * - Kiểm tra nếu user đạt cấp Senior Team Leader trở lên thì cộng thêm hoa hồng Personal Group Bonus (PGB).
//      * - Áp dụng giới hạn 50% hoặc 40% từ chân mạnh tùy vào cấp bậc.
//      * - Nếu có sự thay đổi về BP, cập nhật lại rank.
//      */
//     function updateTeamBP(address user) internal {
...
//         if (totalBP > 0) {
//             // Tính toán phần thưởng PGB nếu user đạt STL trở lên
//             uint256 increasePersonalGroupBP = totalPersonalGroupBP - nodeData[user].personalGroupBP;

//             if (nodes[user].rank >= Rank.SeniorTeamLeader && increasePersonalGroupBP > 0) {
//                 nodeData[user].personalGroupBP = totalPersonalGroupBP; // Cập nhật PGB mới
//                 // balances[user] += (increasePersonalGroupBP * PGrB_PERCENT) / 100; // Trả thưởng cho user
//                 balancesManager.updateBalance(user, (increasePersonalGroupBP * PGrB_PERCENT) / 100, true);
//             }
...
//                 updateRanks(user); // Cập nhật rank
//             }
...
//               updateTeamBP(grandParent); // Cập nhật TeamBP cho cha
//             }
...
//                 if (
//                   hasSales[parent]
//                   && nodes[parent].status != Status.Active
//                   && checkActiveCondition(parent)
//                 ) {
//                     nodes[parent].status = Status.Active;
...
//      */
//     function findActiveParent(address user) internal view returns (address) {
...
//      */
//     function checkActiveCondition(address user) internal view returns (bool) {
//         return nodeData[user].personalBP >= 100 || nodeData[user].teamBP >= 500;
//     }


//     /**
//     * @dev Cập nhật `generationRank` từ root xuống lá cho hệ thống.
//     * 
//     * Thuật toán:
//     * 1. Nếu `current` là `rootNode`:
//     *    - Khởi tạo `generationRank` cho tất cả nhóm rank về `address(0)`.
//     * 
//     * 2. Nếu `current` là một node có rank ảnh hưởng:
//     *    - Rank ảnh hưởng bao gồm: `Director`, `SeniorDirector`, `NationalDirector`.
//     *    - Xét từng `groupIndex` (0 đến 3) tương ứng với các nhóm rank.
//     *    - Nếu là `NationalDirector`:
//     *        - `gen1`, `gen2`, `gen3` đều là `current`.
//     *    - Nếu là `SeniorDirector`:
//     *        - `gen1` và `gen2` là `current`, `gen3` kế thừa từ `gen1` của `parent`.
//     *    - Nếu là `Director`:
//     *        - `gen1` là `current`, `gen2` và `gen3` kế thừa từ `parent`.
//     * 
//     * 3. Nếu `current` không có rank ảnh hưởng:
//     *    - Sao chép `generationRank` từ `parent` cho tất cả nhóm rank.
//     * 
//     * 4. Đệ quy xuống các `children` của `current` để tiếp tục cập nhật `generationRank`.
//     * 
//     * Ưu điểm:
//     * - Tối ưu gas vì không cần duyệt ngược lên từ `current` cho mỗi lần cập nhật.
//     * - Chỉ cập nhật `generationRank` khi cần thiết, kế thừa từ `parent` nếu không cần thay đổi.
//     * - Phù hợp với batch 24h để đồng bộ toàn bộ hệ thống.
//     */

//     function updateGenerationRankFromRoot(address current) internal {
...
//             generationRank[current][groupIndex] = GenerationInfo({
//                 gen1: address(0),
//                 gen2: address(0),
//                 gen3: address(0)
//             });
...
//             if (currentRank == Rank.SeniorDirector) {
//                 gen3 = generationRank[parent][groupIndex].gen1;
//                 gen2 = current;
//                 gen1 = current;
//             }

//             if (currentRank == Rank.Director) {
//                 gen3 = generationRank[parent][groupIndex].gen2;
//                 gen2 = generationRank[parent][groupIndex].gen1;
//                 gen1 = current;
//             }
...
//           for (uint8 groupIndex = 0; groupIndex < 4; groupIndex++) {
//             generationRank[current][groupIndex] = GenerationInfo({
//                 gen1: generationRank[parent][groupIndex].gen1,
...
//         address[] memory children = nodes[current].children;
//         for (uint i = 0; i < children.length; i++) {
//             updateGenerationRankFromRoot(children[i]);
//         }
//     }
...
//             address director = nationalDirectors[i];
            
//             users[counBatchUpdateBalances] = director;
...
//         updateGenerationRankFromRoot(rootNode);
//     }


//     /*
//       Dùng bên ngoài gọi vào hàm mỗi 01h
//     */
//     function hourlyRewardDistribution() external onlyOwner {
...
//         for (uint256 i = 0; i < salesUsers.length; i++) {
//             address user = salesUsers[i];
//             uint256 bp = hourlySales[user].totalBP;

//             address parent = nodes[user].parent;
//             uint8 groupRankIndex = getGroupRank(user);

//             address gen1 = generationRank[parent][groupRankIndex].gen1;
//             if (gen1 != address(0)) {
//                 // balances[gen1] += (bp * GeB_PERCENT) / 100;
...
//             address gen2 = generationRank[parent][groupRankIndex].gen2;
//             if (gen2 != address(0)) {
//                 // balances[gen2] += (bp * GeB_PERCENT) / 100;
...
//             address gen3 = generationRank[parent][groupRankIndex].gen3;
//             if (gen3 != address(0)) {
//                 // balances[gen3] += (bp * GeB_PERCENT) / 100;
...
//             // khi 100 phần tử thì gửi qua cho batchUpdateBalances
//             if (users.length > 100 ) {
//               balancesManager.batchUpdateBalances(users, amounts, isAdd);
//               delete users;  // Reset danh sách users
//               delete amounts;  // Reset danh sách amounts
//               delete isAdd;  // Reset danh sách isAdd

//             }
//             delete hourlySales[user];  // Reset dữ liệu sau khi trả thưởng
...
//         delete salesUsers;  // Reset danh sách user
//     }
...
//                 nodes[user].status = Status.InActive; // Chuyển về InActive
