...
contract TreeCommission is ITreeCommission {
    ...
    */
    ...
    address private daoNode;
    address private stockNode;
    ...
    constructor() payable {
        ...
        // showroomManager = new Showroom();
        // eventLoggerManager = new EventLogger();

        // address _balancesAddress = address(balancesManager);
        ...
        // rootNode = address(new ProposalVote(_rootMembers, _balancesAddress));
        // daoNode = address(new ProposalVote(_daoMembers, _balancesAddress));
        // stockNode = address(new eStock("eStock", "po5", 1000, 1, _usdtAddress));
    ...
    function initialize()external onlyOwner{
        balancesManager = new BalancesManager(msg.sender);
        showroomManager = new Showroom(msg.sender);
        eventLoggerManager = new EventLogger(msg.sender);
    }
    function setNodeAdd(address[] memory _rootMembers, address[] memory _daoMembers, address _usdtAddress)external onlyOwner{
        
        address _balancesAddress = address(balancesManager);
        rootNode = address(new ProposalVote(_rootMembers, _balancesAddress));
        daoNode = address(new ProposalVote(_daoMembers, _balancesAddress));
        stockNode = address(new eStock("eStock", "po5", 1000, 1, _usdtAddress,msg.sender));
        usdtToken = IERC20(_usdtAddress);
        nodes[rootNode].status = Status.Active;
    ...
    function _processPersonalSales(address user, uint256 bp) internal {
         if (vipNodes[user].parent != address(0)){
            balancesManager.updateBalance(user, (bp * RETAIL_COMMISSION_PERCENT) / 100, true);
        }else{
    ...
    */
    ...
    function getInfoVIPNode() external onlyEOA view returns (VIPNode memory) {
        // chỉ cho phép ViewTree gọi
        // require(msg.sender == viewTreeContract, "Unauthorized caller");
        address user = msg.sender;
        require(vipNodes[user].parent != address(0), "User does not exist in VIP");
    ...
    */
    function hourlyRewardDistribution() external onlyOwner {
        ...
        for (uint256 i = 0; i < salesUsers.length; i++) {
            ...
            Rank currentRank = nodes[user].rank;
