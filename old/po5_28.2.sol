...
//         if (nodeData[user].personalBP >= 100 || nodeData[user].teamBP >= 500) {
//             nodes[user].status = TreeLib.Status.Active;
//             return true;
//         }
...
//         address user
//     ) internal {
//         uint256 totalBP = nodeData[user].personalBP;
//         for (uint256 i = 0; i < nodes[user].children.length; i++) {
//             address child = nodes[user].children[i];
//             totalBP += nodeData[child].teamBP + nodeData[child].personalBP;
//         }
//         nodeData[user].totalTeamBP = totalBP;
//     }
...
//      */
//     function findActiveParent(mapping(address => NodeInfo) storage nodes, mapping(address => address) storage activeParentMapping, address user) internal view returns (address) {
...
//     function updateGenerationRank(mapping(address => NodeInfo) storage nodes, mapping(address => mapping(uint8 => GenerationInfo)) storage generationRank, address rootNode, address current) internal {
...
//             updateGenerationRank(nodes, generationRank, rootNode, children[i]);
//         }
//     }


//     function checkTransferUSDT(IERC20 usdtToken, uint256 usdtAmount) internal {
...
//     address[] internal salesUsers;  // Lưu danh sách user phát sinh doanh số
...
//         TreeLib.distributeUnilevelBonus(activeParentMapping, balancesManager, user, bp);
//     }
...
//         TreeLib.checkTransferUSDT(usdtToken, MEMBERSHIP_FEE);
...
//         require(nodes[parent].parent != address(0) || nodes[parent].status == TreeLib.Status.Active, "Parent does not exist"); // Kiểm tra cha có tồn tại và đang active không
...
//         TreeLib.checkTransferUSDT(usdtToken, ACTIVATION_FEE + MEMBERSHIP_FEE);
...
//         require(nodes[parent].parent != address(0) || nodes[parent].status == TreeLib.Status.Active, "Parent does not exist"); // Kiểm tra cha có tồn tại và active không

//         TreeLib.checkTransferUSDT(usdtToken, ACTIVATION_FEE);
...
//         bool isVIP = vipNodes[user].parent != address(0) ? true : false;
//         if (!isVIP) {
...
//      * @param user Địa chỉ thành viên cần cập nhật
//      */
//     function updateActiveStatus(address user) internal {
//         nodes.updateActiveStatus(nodeData, user);
//     }
...
//     function getInfo() external onlyEOA view returns (TreeLib.NodeInfo memory, TreeLib.NodeData memory) {
...
//         nodeData.updateTeamBP(nodes, user);
//     }
...
//          TreeLib.updateGenerationRank(nodes, generationRank, rootNode, rootNode);
//     }
