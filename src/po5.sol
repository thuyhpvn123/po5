...
interface ITreeCommission {
    struct CommissionData {
        uint256 retailBonus;
        uint256 showroomBonus;
        uint256 daoBonus;
        uint256 otherBonus;
        uint256 activationBp;
        uint256 diffPrice;
        uint256 unilevelBonus;
        uint256 generationBonus;
        uint256 nationalBonus;
    }
    enum TYPE_OF_COM {
        RETAIL_COMMISSION,
        DAO_COMMISSION,
...
interface IEComOrder {
    ...
    struct Order {
        ...
        bytes32[] variantIds;
        uint256[] quantities;
        uint256[] diffPrices; // Chênh lệch giá khi user mua hàng bằng tài khoản 
        uint256[] prices; // Giá đã áp dụng (có thể là giá thành viên, VIP, sau chiết khấu)
        uint256[] rewards; // Reward (hoa hồng) cho từng sản phẩm
        uint256 totalPrice;
        uint8 checkoutType;
        uint8 orderStatus;
        bytes32 codeRef;
...
contract BalancesManager is Ownable {
    ...
    function createUltraUTXO(
        ...
        if (userUltraBP[user][utxoID].BP == 0){
            ...
            userUltraBP[user][utxoID].BP += amount;
        }
    }
    ...
    function addCommission(
        address _user,
        uint256 _timestamp,
        ITreeCommission.TYPE_OF_COM _comType,
        uint256 _amount
    ) internal {
    ...
    function updateBalance(address user, bytes32 utxoID, uint256 amount, bool isAdd,ITreeCommission.TYPE_OF_COM _typ) external onlyTreeCommission {
        ...
        addCommission(user,block.timestamp,_typ,amount);
    }

    function batchUpdateBalances(bytes32 utxoID, address[] calldata users, uint256[] calldata amounts, bool isAdd,ITreeCommission.TYPE_OF_COM _typ) external onlyTreeCommission {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            addCommission(users[i],block.timestamp,_typ,amounts[i]);
        }
    }
    ...
    function initUser(address user, int256 longtitude, int256 lattitude) external onlyTreeCommission {
        // require(isLockData == false, "System upgrade"); // Kiểm tra hệ thống đang có nâng cấp không
...
contract Showroom is Ownable {
    ...
    struct ShowroomNode {
        ...
        int256 longtitude;     // Kinh độ showroom
        int256 lattitude;      // Vĩ độ showroom
        address showroom;
    }
     ...
     */
    function addShowRoom(address newShowroom, address parent, ShowroomTier tier, int256 longtitude, int256 lattitude) public onlyOwner {
 ...
 uint256 constant DECIMALS = 1e18;
    uint256 constant R = 6371000; // bán kính Trái đất (mét)
    int256 constant PI = 3141592653589793238; // 3.141592653589793238 scaled 1e18

    // Chuyển độ sang radian fixed point
    function deg2rad(int256 deg) internal pure returns (int256) {
        // rad = deg * pi / 180
        return (deg * PI) / (180 * int256(DECIMALS));
    }

    // Hàm sqrt sử dụng phương pháp Babylonian, input/output fixed point scale 1e18
    ...
    // Hàm sin approximation (x radian fixed point 1e18, trả về fixed point 1e18)
    // approximation sin(x) ≈ x - x^3/6 (đơn giản, dùng cho |x| < pi/2)
    function sin(int256 x) internal pure returns (int256) {
        // sin(x) ≈ x - x^3/6
        int256 x3 = (x * x / int256(DECIMALS)) * x / int256(DECIMALS);
        return x - x3 / 6;
    }

    // Hàm asin approximation (x fixed point 1e18, |x| <= 1)
    // asin(x) ≈ x + x^3/6 (áp dụng cho giá trị nhỏ, khoảng <0.5)
    function asinApprox(uint256 x) internal pure returns (uint256) {
        // x dạng uint256 fixed point 1e18
        uint256 x3 = (x * x) / DECIMALS * x / DECIMALS;
        return x + x3 / 6;
    }

    // Hàm tính khoảng cách theo Haversine
    // lat, lon: int256 fixed point 1e18 (độ * 1e18)
    // trả về khoảng cách mét (uint256)
    function calculateDistance(int256 lat1, int256 lon1, int256 lat2, int256 lon2) public pure returns (uint256) {
        int256 lat1Rad = deg2rad(lat1);
        int256 lat2Rad = deg2rad(lat2);
        int256 dLat = lat2Rad - lat1Rad;
        int256 dLon = deg2rad(lon2 - lon1);

        // sin²(dLat/2)
        int256 sinDLat2 = sin(dLat / 2);
        uint256 sinDLat2Squared = uint256((sinDLat2 * sinDLat2) / int256(DECIMALS));

        // sin²(dLon/2)
        int256 sinDLon2 = sin(dLon / 2);
        uint256 sinDLon2Squared = uint256((sinDLon2 * sinDLon2) / int256(DECIMALS));

        // a = sin²(dLat/2) + cos(lat1)*cos(lat2)*sin²(dLon/2)
        uint256 cosLat1 = uint256(cos(lat1Rad));
        uint256 cosLat2 = uint256(cos(lat2Rad));

        uint256 a = sinDLat2Squared + ((cosLat1 * cosLat2) / DECIMALS) * sinDLon2Squared / DECIMALS;

        // sqrt(a) scaled 1e18
        uint256 sqrtA = sqrt(a * DECIMALS);

        // c = 2 * asin(sqrt(a)) approximate
        uint256 c = 2 * asinApprox(sqrtA);

        // distance = R * c / DECIMALS
        uint256 dist = (R * c) / DECIMALS;
        return dist;
    }

    // Cosine approximation: cos(x) ≈ 1 - x^2 / 2 (x radian fixed point)
    function cos(int256 x) internal pure returns (int256) {
        int256 x2 = (x * x) / int256(DECIMALS);
        return int256(DECIMALS) - x2 / 2;
    }
    /**
     ...
     */
    function findNearestShowroom(int256 userLat, int256 userLon) public view returns (address) {
    ...
    function withdrawBPFromShowRoom(uint256 amount,bytes32[] memory utxoArr) external onlyEOA {
        ...
        showroomNodes[showroom].totalBP = showroomNodes[showroom].totalBP - amount;
    }
...
library TreeLib {
    ...
    function updateActiveStatus(
        ...
        console.log("update active day:",user);
        console.log("kiem tra productBPThisMonth[user]:",productBPThisMonth[user]);
        ...
        console.log("nodeData[user].membershipExpiry:",nodeData[user].membershipExpiry);
        if (nodeData[user].membershipExpiry > 0 && block.timestamp > nodeData[user].membershipExpiry) {
            console.log("bbbbbbb");
    ...
    */
    function isDescendant(
      ...
      address _ancestor,
      uint8 totalLevel,
      mapping(bytes32 => bool) storage linkViewTree,
      bytes32[] storage allLinkViewTreeKeys
    ) internal returns (bool) {
     ...
     */
     function distributeUnilevelBonus(
        ...
        balancesManager.batchUpdateBalances(utxoID, users, amounts, true,ITreeCommission.TYPE_OF_COM.UNILEVEL);
    ...
    function distributeGenerationBonus_batchUpdate(
        ...
        if (users.length > 0 ) {
          balancesManager.batchUpdateBalances(utxoID, users, amounts, true,ITreeCommission.TYPE_OF_COM.GENERATION);
        }
    ...
    //   mapping(address => NodeInfo) storage nodes,
      ...
      address[] storage nationalDirectors
        ...
        balancesManager.batchUpdateBalances(bytes32(0), users, amounts, true,ITreeCommission.TYPE_OF_COM.NATIONAL_COMMISSION);
    ...
    */
    ...
    function checkTransferUSDT(IERC20 usdtToken, bytes32 utxoID, uint256 usdtAmount) internal {
        ...
        if (isValidUTXO(utxoID)) { //thanh toan visa
            ...
            require(receivedUSDT >= usdtAmount - 1e18 && receivedUSDT <= usdtAmount + 1e18, "Incorrect USDT transfer amount");
...
contract TreeCommission is ITreeCommission {
    ...
    // Các phí và bonus được tính theo đơn vị USDT 1e18
    uint256 internal constant ACTIVATION_FEE = 40 * 1e18; // Phí kích hoạt $40
    uint256 internal constant ACTIVATION_BP = 10_000; // 10 BP khi kích hoạt

    uint256 internal constant MEMBERSHIP_FEE = 120 * 1e18; // Phí thành viên $120
    ...
    struct NewMember {
        ...
        address parent;         // Cha
        int256 longtitude;     // Kinh độ
        int256 lattitude;      // Vĩ độ
        bytes32 utxoID;
    }
    ...
    constructor(address _ecomOrderAddress, address _ecomUserAddress, address _balanceManagerAddress, address _showroomAddress, address _eventLoggerAddress, address _rootNodeAddress, address _daoNodeAddress, address _stockNodeAddress, address _usdtAddress) {
        ...
        require(MEMBERSHIP_BP * 1e18 / 10**3 < MEMBERSHIP_FEE, "wrong code");
        ...
        eComOrderManagerAddress = _ecomOrderAddress; 
        eComUserManagerAddress = _ecomUserAddress; 
    ...
    function setShowroom(address _showroom)external onlyOwner{
        showroomManager = Showroom(_showroom);
    }
    ...
    function withdrawBPToUser(uint256 amount, address user,bytes32[] memory utxoArr) internal {
        if(utxoArr.length >0){
            ...
            if(user == msg.sender ){
            uint256 preBalance = balancesManager.getBalance(user);
            require(preBalance >= amount, "Insufficient balance");
            require(balancesManager.withdrawBP(user, amount), "Insufficient balance for withdraw");
            TreeLib.Status userStatus = nodes[user].status;
            require(userStatus != TreeLib.Status.Locked && userStatus != TreeLib.Status.Banned, "Deny access");

            require(amount * 1e18 / 10**3 < usdtToken.balanceOf(address(this)),"Not enough usdt in treeCom");
            ...
            require(usdtToken.transfer(user, amount * 1e18 / 10**3 ), "Transfer failed");
    ...
    function _processPersonalSales(address user, uint256 bp, bytes32 utxoID) internal {
        ...
        console.log("productBPThisMonth[user]:",productBPThisMonth[user]);
        ...
        if (nodes[user].status != TreeLib.Status.Locked) {
          balancesManager.updateBalance(user, utxoID, (bp * commissionPercent) / 100, true,TYPE_OF_COM.RETAIL_COMMISSION);
        }
        ...
        // hoa hồng cho DAO 4%
        balancesManager.updateBalance(daoNode, utxoID, (newBP * DAO_COMMISSION_PERCENT) / 100, true,TYPE_OF_COM.DAO_COMMISSION);


        // hoa hồng cho stock
        balancesManager.updateBalance(stockNode, utxoID, stockNodeBonus, true,TYPE_OF_COM.OTHER);
    ...
    function addVIPMember(address newMember, address parent, bytes32 utxoID) external onlyEOA {
        ...
        // Lấy thông tin đơn hàng từ smart contract ECOM
        require(parent == eComUserManager.getUserParent(newMember), "parent not match");
        ...
        balancesManager.updateBalance(stockNode, utxoID, MEMBERSHIP_FEE / 1e18 * 10**3 - MEMBERSHIP_BP, true,TYPE_OF_COM.OTHER);
     ...
     */
    function addPromoterMember(address newMember, address parent, int256 longtitude, int256 lattitude, bytes32 utxoID) external onlyEOA {
        require(isLockData == false, "System upgrade"); // Kiểm tra hệ thống đang có nâng cấp không
     ...
     */
    function upgradeToPromoter(address user, int256 longtitude, int256 lattitude, bytes32 utxoID) external payable onlyEOA {
        require(isLockData == false, "System upgrade"); // Kiểm tra hệ thống có đang nâng cấp không
        ...
        require(nodes[parent].parent != address(0) || parent == rootNode, "Parent does not exist"); // Kiểm tra cha có tồn tại và active không
     ...
     * @param lattitude Vĩ độ vị trí thành viên mới
     */
    function _processNewMember(address newMember, address parent, int256 longtitude, int256 lattitude, bytes32 utxoID) internal {
        ...
        console.log("user newmember la:",newMember);
        console.log("nodeData[newMember].membershipExpiry la:",nodeData[newMember].membershipExpiry);
        ...
        balancesManager.updateBalance(stockNode, utxoID, ACTIVATION_FEE / 1e18 * 10**3 - (SHOWROOM_BONUS + ACTIVATION_BP), true,TYPE_OF_COM.OTHER);
    }
    ...
    function buyProduct(bytes32 orderCode, bytes32 utxoID) external onlyEOA {
        require(isLockData == false, "System upgrade"); // Kiểm tra hệ thống có đang nâng cấp không
        require(orderCode != bytes32(0), "wrong order Code");
        
        IEComOrder eComManager = IEComOrder(eComOrderManagerAddress);
        // Lấy thông tin đơn hàng từ smart contract ECOM
        uint256 orderID = eComManager.getOrderCode(orderCode);
        require(orderID != 0, "Order not found");

        IEComOrder.Order memory order = eComManager.getOrder(orderID);

        uint256 amount = order.totalPrice;

        uint256 diffPrice = 0; // diffPrice là trả 80% cho member bán, 20% cho DAO
        uint256 bp = 0;
        for (uint8 i = 0; i < order.rewards.length; i++) {
            require(order.rewards[i] > 0, "reward fail");
            bp += order.rewards[i];
            diffPrice += order.diffPrices[i];
        ...
        // uint256 commission = amount / 1e18 - bp;
        // require(commission > 0, "Wrong commission");
        require(amount / 1e18 > bp, "Wrong commission");
        ...
        // } else {
        //     console.log("user buy product la:",user);
        ...
        // }
        //thanh:
        if (nodes[user].parent == address(0) || nodes[user].status != TreeLib.Status.Active) {// nếu user ko phải là Promoter, thì doanh số tính cho parent
          // tính doanh số cho parent
          user = parent;
            // require(nodes[parent].parent != address(0), "Parent is not exists"); // Kiểm tra cha có đang active không
            require(parent == rootNode || nodes[parent].parent != address(0), "Parent is not exists"); // Kiểm tra cha có đang active không
        } else {
            console.log("user buy product la:",user);
          require(nodes[user].parent == parent, "Parent not match");
        ...
        eComManager.ConfirmPaymentForOrderCode(orderCode,msg.sender);
    ...
    function updateDeliveryProduct(bytes32 orderCode) external onlyAddress(eComOrderManagerAddress) {
        ...
        // Lấy thông tin đơn hàng từ smart contract ECOM
        uint256 bp = confirmPayment[orderCode].BP;
        require(bp > 0, "Order not found");

        bytes32 utxoID = confirmPayment[orderCode].utxoID;
        uint256 diffPrice = confirmPayment[orderCode].diffPrice;
        address user = confirmPayment[orderCode].user;

        if (diffPrice > 0) {
          if (nodes[user].status != TreeLib.Status.Locked) {
            balancesManager.updateBalance(user, utxoID, (diffPrice / 1e18) * 80 / 100, true,TYPE_OF_COM.DIFFPRICE);
          }
          // trả cho DAO 20% số tiền chênh lệch giữa giá member và giá vip
            balancesManager.updateBalance(daoNode, utxoID, (diffPrice / 1e18) * 20 / 100, true,TYPE_OF_COM.DIFFPRICE);
        ...
        balancesManager.updateBalance(rootNode, utxoID, bp, true,TYPE_OF_COM.OTHER);
    }
