...
contract DateTimeTZ {
    ...
    function addTimezone(uint timestamp, int tz) public pure returns (uint) {
        int newTimestamp = int(timestamp) + int(DateTimeLibrary.SECONDS_PER_HOUR) * tz;
        require(newTimestamp > 0);
        return uint(newTimestamp);
    }

    function timestampToDateTime(uint timestamp, int tz) public pure returns (uint year, uint month, uint day, uint hour, uint minute, uint second) {
        // Convert GTM+0 to TZ
        timestamp = addTimezone(timestamp, tz);
        return DateTimeLibrary.timestampToDateTime(timestamp);
    }

    function timestampToDate(uint timestamp, int tz) public pure returns (uint year, uint month, uint day) {
        // Convert GTM+0 to TZ
        timestamp = addTimezone(timestamp, tz);
        return DateTimeLibrary.timestampToDate(timestamp);
    }

    function getYMD(uint timestamp, int tz)  public pure returns (uint) {
        (uint y, uint m, uint d) = timestampToDate(timestamp, tz);
        return y * 10000 + m * 100 + d;
    }

    function getYM(uint timestamp, int tz)  public pure returns (uint) {
        (uint y, uint m, ) = timestampToDate(timestamp, tz);
        return y * 100 + m;
    }

    function timestampFromDate(uint year, uint month, uint day, int tz) public pure returns (uint timestamp) {
    ...
    function timestampFromDateTime(uint year, uint month, uint day, uint hour, uint minute, uint second, int tz) public pure returns (uint timestamp) {
        timestamp = DateTimeLibrary.timestampFromDateTime(year, month, day, hour, minute, second);
        // Convert TZ to GTM+0
        timestamp = addTimezone(timestamp, -tz);
    }

    function getStartOfDate(uint timestamp, int tz) public pure returns (uint startOfDate) {
        (uint year, uint month, uint day) = timestampToDate(timestamp, tz);
        startOfDate = timestampFromDate(year, month, day, tz);
    }

    function getEndOfDate(uint timestamp, int tz) public pure returns (uint endOfDate) {
    ...
    function extractYMD(uint _ymd) public pure returns (uint year, uint month, uint day) {
        year = _ymd / 10000;
        month = (_ymd / 100) % 100;
        day = _ymd % 100;
    }

    function timestampFromYMD(uint _ymd, int tz) public pure returns (uint timestamp) {
        (uint year, uint month, uint day) = extractYMD(_ymd);
        return timestampFromDate(year, month, day, tz);
    }
    function timestampFromYMD(uint year, uint month, uint day, int tz) public pure returns (uint timestamp) {
        return timestampFromDate(year, month, day, tz);
    }

    function getDaysInMonth(uint year, uint month) public pure returns (uint daysInMonth) {
        daysInMonth = DateTimeLibrary.getDaysInMonth(year, month);
    }

    function getDayOfWeekFromYMD(uint _ymd, int tz) public pure returns (uint dayOfWeek) {
        uint timestamp = timestampFromYMD(_ymd, tz);
        timestamp = addTimezone(timestamp, tz);
        return DateTimeLibrary.getDayOfWeek(timestamp);
    }

    function getYYYYMMDDAndTime(uint timestamp, int tz) public pure returns (uint ymd, uint time) {
        ymd = getYMD(timestamp, tz);
        time = timestamp - getStartOfDate(timestamp, tz);
    }

    function timestampToString(uint timestamp, int tz) public pure returns (string memory) {
        (uint year, uint month, uint day) = timestampToDate(timestamp, tz);
        return string.concat(Strings.toString(day), "/", Strings.toString(month), "/", Strings.toString(year));
    }

    function ymdToString(uint ymd) public pure returns (string memory) {
        (uint year, uint month, uint day) = extractYMD(ymd);
        return string.concat(Strings.toString(day), "/", Strings.toString(month), "/", Strings.toString(year));
    }

    function timestampToDetailString(uint timestamp, int tz) public pure returns (string memory) {
    ...
    function timestampToDateAndTimeString(uint timestamp, int tz) public pure returns (string memory date, string memory time) {
        (uint year, uint month, uint day, uint hour, uint minute,) = timestampToDateTime(timestamp, tz);
